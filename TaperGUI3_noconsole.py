...
class PerformTaper():
    def __init__(self, gui, xlStart, xlEnd, xrStart, xrEnd, xlxr, vlxr, vrxr, arcxr, name, rotation=False, arc=False):
        self.started = False
        self.name = name
...
#==============================================================================
    ...
    def Begin(self):
        ...
        if abs(self.xlStart-curpos[0])+abs(self.xrStart-curpos[1])<500:
            if self.rotation:
                splicer.spin(-500000,speed=0.1)  #NEGATIVE SPIN SEEMS TO BE MUCH BETTER THAN POSITIVE
...
class MeasureTaper():
    ...
    def __init__(self,gui,measureStart,measureEnd,extra = 2000.,fname='nonenone'): 
...
taper += parabolic(4000,ED,SD)""")
    ...
    def okClicked(self):
        ...
        ts = taperSections
        d = {'np':np, 'linear':ts.linear,'sine':ts.sinewave,'parabolic':ts.parabolic,'taper':ts.Taper(), 'newTaper':ts.Taper}
        try:
    ...
    def measureClicked(self):
        ...
        text, ok = QtGui.QInputDialog.getText(self, 'Input Dialog', 
            'Enter Directory Prefix for Saving:',text='taper1')
        #fname = 'fartfart'
        if ok:
            fname = str(text)
            
        self.Measurement =  MeasureTaper(self, self.taper.xlEnd, self.taper.xlStart, 1000, fname)

...
class PerformTaper():
    def __init__(self, gui, xlStart, 
                 xlEnd, xrStart, xrEnd, xlxr, 
                 vlxr, vrxr, arcxr, name, 
                 rotation=False, arc=False,spinspeed=0.1):
        self.started = False
        self.name = name
...
taper += parabolic(4000,ED,SD)""")
    ...
    def okClicked(self):
        ...
        ts = taperSections
        d = {'np':np, 'linear':ts.linear,
        'sine':ts.sinewave,
        'parabolic':ts.parabolic,
        'taper':ts.Taper(),
        'newTaper':ts.Taper}
        try:
        ...
        self.spinspeed = d.get('spinspeed',0.1)
